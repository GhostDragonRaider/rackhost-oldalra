import type { AuditSeverity } from "../../../lib/website-audit/types";
...
export function ScoreRing({ score, label, size = 148 }: ScoreRingProps) {
  ...
  return (
    <div
      ...
      role="img"
      aria-label={`Összesített pontszám: ${clamped} per 100, ${label}`}
    >
  ...
  return (
    <div className="audit-sev-dist" aria-label="Súlyosság eloszlás">
      ...
      <ul className="audit-sev-dist__list">
        {items.map((i) => (
          <li key={i.key} className={`audit-sev-chip audit-sev--${i.key}`}>
